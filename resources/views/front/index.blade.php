...
@section('content')
        ...
        <div class="mb-10 text-center">
            <div class="flex items-center justify-center mb-3">
                <img src="{{ asset('asset/images/logo.png') }}" alt="Logo"
                    class="h-16 w-16 rounded-full shadow-lg border-2 border-black">
            </div>
                        ...
                        <table class="w-full text-sm sm:text-base">
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4m16 0l-4 4m4-4l-4-4" />
                                    ...
                                    <td class="p-h ph product-rate border-b" style="width: 25%; text-align: center;">
                                        <input type="number" class="border border-yellow-600" name="gram" min="1"
                                            onkeyup="
                                    let gram = this.value;
                                    if(gram == ''){
                                        const price = ({{ $prices->gram_in_inr }} - (({{ $prices->gram_in_inr }} * 8)/100)) * 1;
                    ...
                    <div
                        ...
                        <table class="left-side-table">
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4m16 0l-4 4m4-4l-4-4" />
                                    ...
                                    <td class="p-h ph product-rate border-b border-yellow-800"
                                        style="width: 25%; text-align: center;">
                                        <input type="number" class="border border-yellow-800" name="gram" min="1"
                                            onkeyup="
                                    let gram = this.value;
                                    if(gram == ''){
                                        const price = ({{ $prices->gram_in_inr }} - (({{ $prices->gram_in_inr }} * 16.7)/100)) * 1;
    ...
    <div class="max-w-4xl mx-auto">
        <div class="space-y-6">
            <!-- Header Table -->
            <div id="divFutureHd" class="bg-gradient-to-r from-amber-600 via-[#FAF3BC] to-amber-600 text-black border border-yellow-200 rounded-xl shadow-md overflow-hidden">
                ...
                <table class="w-full text-sm md:text-base text-center text-gray-700">
                    <tbody class="divide-y divide-gray-100">
                        <tr class="hover:bg-gray-50">
                            ...
                            <td><span class="e gold-bid font-mono text-green-600">{{ $prices->ounce_price_bid }}</span></td>
                            <td><span class="e gold-ask font-mono text-red-600">{{ $prices->ounce_price_ask }}</span></td>
                            <td><span class="e gold-high-low">{{ $prices->ounce_price_usd_today_low }} / {{ $prices->ounce_price_usd_today_high }}</span></td>
                        </tr>
                        <tr class="hover:bg-gray-50">
                            <td class="py-3 font-semibold text-yellow-800">Silver $</td>
                            <td><span class="e silver-bid font-mono text-green-600">{{ round($silverPrices->silver_ounce_price_bid_usd, 2) }}</span></td>
                            <td><span class="e silver-ask font-mono text-red-600">{{ round($silverPrices->silver_ounce_price_ask_usd, 2) }}</span></td>
                            <td><span class="e silver-high-low">{{ round($silverPrices->silver_ounce_price_usd_today_low_usd, 2) }} / {{ round($silverPrices->silver_ounce_price_usd_today_high_usd, 2) }}</span></td>
                        </tr>
                        <tr class="hover:bg-gray-50">
                            <td class="py-3 font-semibold text-yellow-800">INR ₹</td>
                            <td colspan="2"><span class="e inr-usd font-mono">{{ $prices->usd_to_inr }}</span></td>
                            <td>
                                <span class="e gram-usd">{{ $prices->gram_in_usd }}</span><br>
                                <span class="e inr-gram-pair text-sm text-gray-500">{{ $prices->usd_to_inr }}/{{ $prices->gram_in_usd }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
